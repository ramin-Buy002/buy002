...
const Trade = () => {
  const [dataCoins, setDataCoins] = useState([
    ...
    { nameCoin: "HYPE", first_pcs: 10, now_pcs: 11, firstPrice: 46.406, nowPrice: 0, buy_sell_price: 50.01 },
  ]);
  ...
  if(  sell_Data.first_pcs * sell_Data.firstPrice * 0.25 < sell_Data.now_pcs * sell_Data.nowPrice  ) {
     setHi_percent_coin({ sell: sellCoin, buy: buyCoin, percent: best.percent })
  } else {     setHi_percent_coin({ })} ;
    ...
    const fetchFromHyperliquid = async () => {
      ...
      try {
        ...
        const updated = dataCoins.map(coin => {
          ...
          if (idx >= 0) {
            const ctx = ctxs[idx];
            coin.nowPrice = parseFloat(ctx.markPx || ctx.markPrice || ctx.oraclePx || 0);
          }
          return coin;
        });
        ...
        <div className="ml-40">
          <ul>
            {dataCoins.map((r, index) => (
              <p key={index} className="ml-2 mt-2 bg-blue-50 flex justify-between border rounded-lg p-3 w-[680px]">
                <span className="text-emerald-950 font-bold ml-3">{r.nameCoin}</span>
                <span className="ml-6">now_price: <b>{r.nowPrice}</b> $</span>
                <span className="ml-8">now_pcs: <b>{r.now_pcs}</b></span>
                <span className="ml-8">first_Price: <b>{r.firstPrice}</b></span>
                <span className="ml-8">first_pcs: <b>{r.first_pcs}</b></span>
                <span className="ml-8 mr-9">buy_sell_price: <b>{r.buy_sell_price}</b></span>
              </p>
            ))}
          </ul>
          ...
          <div className="flex flex-row ml-10">
            ...
            <p   >
                                  ...
                                  <span   className="    ml-3  text-2xl font-bold " >  {a} {"   "} $ </span>
                      </p>
                      ...
                      <p  className="  mt-3 " >
                                  ...
                                  <span className=" ml-3   text-2xl font-bold "  >  {b} {"   "} $ </span>
                      </p>
                      ...
                      <p className="  mt-3 " >
                                  ...
                                  <span className=" ml-3    text-2xl font-bold " >  {c} {"   "} $ </span>
                      ...
                      <p  className="  mt-3 " >
                                    ...
                                    <span className=" ml-3   text-2xl font-bold " > {d} {"   "} $ </span>
